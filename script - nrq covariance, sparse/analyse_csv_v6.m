...
%         b = find(simulation_cases.nGridTime(iCase));
        ...
        truncation_max = max(subdata.transfer_TruncationParameter);
                
        vec = [];
        vec.transfer_RMSE = zeros(1,truncation_max);
        vec.ZErrorTrain = zeros(1,truncation_max);
        vec.ZErrorTrain_true = zeros(1,truncation_max);
        vec.ZErrorTest = zeros(1,truncation_max);
        vec.ZErrorTest_true = zeros(1,truncation_max);
        vec.ZErrorTrain_holdout_ = zeros(1,truncation_max);
        
        for k = 1:truncation_max
            vec.transfer_RMSE(k) = median( subdata.transfer_RMSE( subdata.transfer_TruncationParameter == k ) );
            vec.ZErrorTrain(k) = median( subdata.ZErrorTrain( subdata.transfer_TruncationParameter == k ) );
            vec.ZErrorTrain_true(k) = median( subdata.ZErrorTrain_trueDynamics_( subdata.transfer_TruncationParameter == k ) );
            vec.ZErrorTest(k) = median( subdata.ZErrorTest( subdata.transfer_TruncationParameter == k ) );
            vec.ZErrorTest_true(k) = median( subdata.ZErrorTest_trueDynamics_( subdata.transfer_TruncationParameter == k ) );
            vec.vec.ZErrorTrain_holdout_(k) = median( subdata.ZErrorTrain_holdout_( subdata.transfer_TruncationParameter == k ) );
        end
...
subplot(3,3,9)
h = heatmap(xvalues,yvalues,square.ZErrorTrain_holdout_best_k);
h.Title = 'ZErrorTrain_holdout_best_k';
h.XLabel = 'avg number of obs. per curve';
h.YLabel = 'T';
