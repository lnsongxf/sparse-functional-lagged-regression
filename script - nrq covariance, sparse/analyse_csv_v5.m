...
%         b = find(simulation_cases.nGridTime(iCase));
        ...
        tikhonov_params = unique(subdata.transfer_TikhonovParameter);
        tikhonov_params_n = length(tikhonov_params);
                
        vec = [];
        vec.transfer_RMSE = zeros(1,tikhonov_params_n);
        vec.ZErrorTrain = zeros(1,tikhonov_params_n);
        vec.ZErrorTrain_true = zeros(1,tikhonov_params_n);
        vec.ZErrorTest = zeros(1,tikhonov_params_n);
        vec.ZErrorTest_true = zeros(1,tikhonov_params_n);
        vec.ZErrorTrain_holdout_ = zeros(1,tikhonov_params_n);
        
        for k = 1:tikhonov_params_n
            rho = tikhonov_params(k);
            vec.transfer_RMSE(k) = median( subdata.transfer_RMSE( subdata.transfer_TikhonovParameter == rho ) );
            vec.ZErrorTrain(k) = median( subdata.ZErrorTrain( subdata.transfer_TikhonovParameter == rho ) );
            vec.ZErrorTrain_true(k) = median( subdata.ZErrorTrain_trueDynamics_( subdata.transfer_TikhonovParameter == rho ) );
            vec.ZErrorTest(k) = median( subdata.ZErrorTest( subdata.transfer_TikhonovParameter == rho ) );
            vec.ZErrorTest_true(k) = median( subdata.ZErrorTest_trueDynamics_( subdata.transfer_TikhonovParameter == rho ) );
            vec.vec.ZErrorTrain_holdout_(k) = median( subdata.ZErrorTrain_holdout_( subdata.transfer_TikhonovParameter == rho ) );
        end
        ...
        square.transfer_RMSE_best_rho(a,b) = tikhonov_params(square.transfer_RMSE_best_k(a,b));
        square.ZErrorTrain_best_rho(a,b) = tikhonov_params(square.ZErrorTrain_best_k(a,b));
        square.ZErrorTest_best_rho(a,b) = tikhonov_params(square.ZErrorTest_best_k(a,b));
        square.ZErrorTrain_holdout_best_rho(a,b) = tikhonov_params(square.ZErrorTrain_holdout_best_k(a,b));
...
subplot(3,3,2)
h = heatmap(xvalues,yvalues,square.transfer_RMSE_best_rho);
h.Title = 'transfer RMSE - best rho';
h.XLabel = 'avg number of obs. per curve';
h.YLabel = 'T';
% 
% subplot(3,3,4)
...
subplot(3,3,7)
h = heatmap(xvalues,yvalues,square.ZErrorTrain_best_rho);
h.Title = 'ZErrorTrain_best_rho';
h.XLabel = 'avg number of obs. per curve';
h.YLabel = 'T';
% 
% subplot(3,3,5)
...
subplot(3,3,9)
h = heatmap(xvalues,yvalues,square.ZErrorTrain_holdout_best_rho);
h.Title = 'ZErrorTrain_holdout_best_rho';
h.XLabel = 'avg number of obs. per curve';
h.YLabel = 'T';
...
% what rho I'm doing by default

my_rho = 2 .* square.nGridTime.^(-1/3) .* (square.nRandi_max./2).^(-1/2)
% my_rho = 2 .* square.nGridTime.^(-1/3) .* (square.nRandi_max./2).^(-1/4) % this is in the simulations of 22.02.


subplot(3,3,3)
h = heatmap(xvalues,yvalues, my_rho);
h.Title = 'my_rho';
h.XLabel = 'avg number of obs. per curve';
h.YLabel = 'T';
...
subplot(3,3,4)
h = heatmap(xvalues,yvalues, square.ZErrorTrain_holdout_best_rho ./ my_rho);
h.Title = 'ratio between best (for Z error holdout) and my rho';
h.XLabel = 'avg number of obs. per curve';
h.YLabel = 'T';

subplot(3,3,5)
h = heatmap(xvalues,yvalues, square.transfer_RMSE_best_rho./ my_rho);
h.Title = 'ratio between best (for transfer RMSE) and my rho';
h.XLabel = 'avg number of obs. per curve';
h.YLabel = 'T';

%% subselect according to my rho
