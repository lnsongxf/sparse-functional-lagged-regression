...
square.specCrossDensityRMSE_L2 = nan(3,5);
...
square_truncation.transfer_RMSE = nan(3,5);
square_truncation.transfer_RMSE_sd = nan(3,5);
square_truncation.ZErrorTrain = nan(3,5);
square_truncation.ZErrorTrain_true = nan(3,5);
square_truncation.ZErrorTest = nan(3,5);
square_truncation.ZErrorTrain_holdout_ = nan(3,5);
square_truncation.ZErrorTrain_holdoutTrueDynamics_ = nan(3,5);

square_Tikhonov.transfer_RMSE = nan(3,5);
square_Tikhonov.transfer_RMSE_sd = nan(3,5);
square_Tikhonov.ZErrorTrain = nan(3,5);
square_Tikhonov.ZErrorTrain_true = nan(3,5);
square_Tikhonov.ZErrorTest = nan(3,5);
square_Tikhonov.ZErrorTrain_holdout_ = nan(3,5);
square_Tikhonov.ZErrorTrain_holdoutTrueDynamics_ = nan(3,5);
...
%         square.nGridTime(a,b) = subdata_truncation.T(1);
        ...
        square.covLag0RMSE(a,b) = mean( subdata_truncation.covLag0RMSE);
        
        square.specDensityRMSE_FRO(a,b) = mean( subdata_truncation.specDensityRMSE_FRO);
        square.specDensityRMSE_FRO_sd(a,b) = sqrt(var( subdata_truncation.specDensityRMSE_FRO));
        
        square.specDensityRMSE_FRO(a,b) = mean( subdata_truncation.specDensityRMSE_FRO);
        square.specCrossDensityRMSE_L2(a,b) = mean( subdata_truncation.specCrossDensityRMSE_L2 );
        
        square.dynamicKrigRMSE(a,b) = mean( subdata_truncation.dynamicKrigRMSE(subdata_truncation.sigma_2EstimLocQv_used_>0.01) );
        square.dynamicKrigRMSE_IQR(a,b) = iqr( subdata_truncation.dynamicKrigRMSE(subdata_truncation.sigma_2EstimLocQv_used_>0.01) );
        %     square.staticKrigRMSE(a,b) = mean( data.staticKrigRMSE(data.sigma_2EstimLocQv_used_>0.05) );
        %     square.staticKrigRMSE_IQR(a,b) = iqr( data.staticKrigRMSE(data.sigma_2EstimLocQv_used_>0.05) );
        
        square.percentage_good_sigma(a,b) = mean(subdata_truncation.sigma_2EstimLocQv_used_>0.01)*100;
        
        square.sigma_2EstimLocLin(a,b) = mean( subdata_truncation.sigma_2EstimLocLin.^2);
        square.sigma_2EstimLocQv_used_(a,b) = mean( subdata_truncation.sigma_2EstimLocQv_used_.^2);
   
        % outputs for the 2nd paper
        square_truncation.transfer_RMSE(a,b) = mean( subdata_truncation.transfer_RMSE );
        square_truncation.transfer_RMSE_sd(a,b) = var( subdata_truncation.transfer_RMSE )^(1/2);
        square_truncation.ZErrorTrain(a,b) = mean( subdata_truncation.ZErrorTrain );
        square_truncation.ZErrorTrain_true(a,b) = mean( subdata_truncation.ZErrorTrain_trueDynamics_ );
        square_truncation.ZErrorTest(a,b) = mean( subdata_truncation.ZErrorTest );
        square_truncation.ZErrorTrain_holdout_(a,b) = mean( subdata_truncation.ZErrorTrain_holdout_ );
        square_truncation.ZErrorTrain_holdoutTrueDynamics_(a,b) = mean( subdata_truncation.ZErrorTrain_holdoutTrueDynamics_ );
        
        square_Tikhonov.transfer_RMSE(a,b) = mean( subdata_Tikhonov.transfer_RMSE );
        square_Tikhonov.transfer_RMSE_sd(a,b) = var( subdata_Tikhonov.transfer_RMSE )^(1/2);
        square_Tikhonov.ZErrorTrain(a,b) = mean( subdata_Tikhonov.ZErrorTrain );
        square_Tikhonov.ZErrorTrain_true(a,b) = mean( subdata_Tikhonov.ZErrorTrain_trueDynamics_ );
        square_Tikhonov.ZErrorTest(a,b) = mean( subdata_Tikhonov.ZErrorTest );
        square_Tikhonov.ZErrorTrain_holdout_(a,b) = mean( subdata_Tikhonov.ZErrorTrain_holdout_ );
        square_Tikhonov.ZErrorTrain_holdoutTrueDynamics_(a,b) = mean( subdata_Tikhonov.ZErrorTrain_holdoutTrueDynamics_ );
...
for ii = 1:3
    ...
    deltaB_by_sp_truncation(ii) = mean( subdata_truncation.transfer_RMSE );
    deltaB_by_sp_Tikhonov(ii) = mean( subdata_Tikhonov.transfer_RMSE );
    deltaPred_by_sp_truncation(ii) = mean( subdata_truncation.ZErrorTrain_holdout_ );
    deltaPred_by_sp_Tikhonov(ii) = mean( subdata_Tikhonov.ZErrorTrain_holdout_ );
    deltaPred_by_sp_true(ii) = mean( subdata_Tikhonov.ZErrorTrain_holdoutTrueDynamics_ );
    ...
    deltaB_by_rp_truncation(ii) = mean( subdata_truncation.transfer_RMSE );
    deltaB_by_rp_Tikhonov(ii) = mean( subdata_Tikhonov.transfer_RMSE );
    deltaPred_by_rp_truncation(ii) = mean( subdata_truncation.ZErrorTrain_holdout_ );
    deltaPred_by_rp_Tikhonov(ii) = mean( subdata_Tikhonov.ZErrorTrain_holdout_ );
    deltaPred_by_rp_true(ii) = mean( subdata_Tikhonov.ZErrorTrain_holdoutTrueDynamics_ );
end
