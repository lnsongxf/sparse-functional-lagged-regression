function [censor, dataFull, dataFullONB, dataFull_padded, dataFullONB_padded, covs_true, mu_true, specDensity_true,maxEigenvalue,sigma] = fSimulate_AR(onb,simulated_process,nGridTime,nRandi_max,nRandi_min,nGridFreq,dataFull_padding)
...
% surf(parA0_inSpace,'EdgeColor','none')
...
% stochCov_inSpace = 0.7 * 2*sin(2*pi*g')*sin(2*pi*g) + 0.3*2*cos(2*pi*g')*cos(2*pi*g); % more spiky
% stochCov_inSpace = 5*exp(-(g'-g).^2 *10);

%% set up the parameters
g = onb.gridSpace;

% covariance of stochastic term
% par_covE_inSpace = 5*exp(-(g'-g).^2 *10); % original
% norm(par_covE_inSpace) = 576
alpha = 1;
ell = 0.2;
r = ( g.^2 - g.^2');
par_covE_inSpace = ( 1 + r.^2/(2*alpha*ell^2) ).^(-alpha);

% surf(par_covE_inSpace, 'EdgeColor', 'none')
% zlim([0 1])
% eig(par_covE_inSpace)
% sum(eig(par_covE_inSpace))
% v = eig(par_covE_inSpace) / sum(eig(par_covE_inSpace));
% v = sort(v,'desc');
% v = cumsum(v);
% v(1:10)
% norm(par_covE_inSpace)
%%

stochCov_inSpace = par_covE_inSpace;
parS0 = onb.onbMatrix'*stochCov_inSpace*onb.onbMatrix / onb.nGridSpace^2;

% %% common mean function
% parMU_inSpace = 4*sin(onb.gridSpace*1.5*pi); % in the space variable
