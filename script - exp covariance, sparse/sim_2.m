function sim_2(initial_seed,simulated_process,response_process,iCase) % XXX
...
fileName_tikhonov = 'exp_sim_tikhonov.csv';
fileName_threshold_value = 'exp_sim_threshold_value.csv';
...
specDensity = fEstimateSpecDensity_v5(censor,onb,mu_est_inSpace,bw_F,nGridFreq);
...
tikhonovParameter = 4* censor.nGridTime ^(-1/3) * (sum(censor.nGrid)/nGridTime)^(-1/2);
disp( ['spectral transfer, tikhonov parameter = ',num2str(tikhonovParameter)] )
transfer = fEstimateTransfer_Tikhonov(onb,specDensity,specCrossDensity, tikhonovParameter);
